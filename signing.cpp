...
uint32_t SlotParseGeneralHeader(const char *szSlotName, const uint8_t *pSlotBase, CS_BlobIndex *pbi) {
...
bool SlotBuildRequirements(const string &strBundleID, const string &strSubjectCN, string &strOutput) {
    ...
    auto uBundldIDLength = (uint32_t) strBundleID.size();
    ...
    auto uSubjectCNLength = (uint32_t) strSubjectCN.size();
...
bool SlotBuildCodeDirectory(bool bAlternate,
    ...
    auto uPageSize = (uint32_t) pow(2, cdHeader.pageSize);
    ...
    if (nullptr != pCodeSlotsData && (uCodeSlotsDataLength == uCodeSlots * cdHeader.hashSize)) { //use exists
...
bool ParseCodeSignature(uint8_t *pCSBase) {
    ...
    auto *pbi = (CS_BlobIndex *) (pCSBase + sizeof(CS_SuperBlob));
...
bool GetCodeSignatureExistsCodeSlotsData(uint8_t *pCSBase,
    ...
    return ((nullptr != pCodeSlots1Data) && (nullptr != pCodeSlots256Data) && uCodeSlots1DataLength > 0 &&
            uCodeSlots256DataLength > 0);
}
